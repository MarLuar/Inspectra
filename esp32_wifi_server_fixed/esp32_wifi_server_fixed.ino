...
String getProjectQRCode(String projectName) {
  for (int i = 0; i < projectCount; i++) {
    if (projectQRs[i].name == projectName) {
      ...
      if (storedQR != "") {
        // Check if stored QR code is in the format "INSPROJECT:uuid|projectname"
        if (storedQR.startsWith("INSPROJECT:")) {
          // Extract the project ID from the stored format "INSPROJECT:uuid|projectname"
          int pipePos = storedQR.indexOf("|");
          if (pipePos != -1) {
            String projectId = storedQR.substring(11, pipePos); // "INSPROJECT:" is 11 chars
            String actualProjectName = storedQR.substring(pipePos + 1);
            String ipAddress = WiFi.localIP().toString();
            String qrCodeUrl = "http://" + ipAddress + "/list_files?project_name=" + actualProjectName;
            Serial.println("Converted stored QR code format to URL: " + qrCodeUrl);
            return qrCodeUrl;
          }
        }
        // If not in special format, return as is (might be a direct URL)
        return storedQR;
      }
    }
  }
