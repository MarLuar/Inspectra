...
// Recursive helper function to collect all files from all directories
void collectAllFilesRecursiveInternal(String prefix, JsonArray targetArray) {
  String response = getFileListFromEndpointWithPrefix(prefix);

  DynamicJsonDocument doc(4096);
  DeserializationError error = deserializeJson(doc, response);

  if (error) {
    Serial.println("JSON parsing failed for prefix: " + prefix + ", error: " + String(error.c_str()));
    return;
  }
  ...
  for (JsonObject item : items) {
    ...
    if (isDirectory) {
      ...
      String subDirPath = prefix + name;
      Serial.println("Found directory: " + subDirPath + ", exploring...");
      collectAllFilesRecursiveInternal(subDirPath + "/", targetArray);
    } else {
      // If it's a file, add it to the target array with the full path
      JsonObject newItem = targetArray.createNestedObject();
      newItem["name"] = prefix + name;  // Full path
      newItem["id"] = item["id"];
...
// Function to get ALL files from Supabase storage (recursive)
String getAllFilesFromStorage() {
  DynamicJsonDocument resultDoc(8192); // Larger document to hold all files
  JsonArray resultArray = resultDoc.to<JsonArray>();

  // Start collecting files from the root
  collectAllFilesRecursiveInternal("", resultArray);

  String result;
  serializeJson(resultArray, result);
  Serial.println("Collected all files, total count: " + String(resultArray.size()));
  return result;
}

// Function to get file list from Supabase storage (wrapper for recursive function)
String getFileListFromEndpoint() {
  Serial.println("Getting ALL files from Supabase storage (recursive)...");
